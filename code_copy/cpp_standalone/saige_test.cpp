...
void SAIGEClass::scoreTest(arma::vec & t_GVec,
	...
	t_islogp = false;
        ...
        boost::math::chi_squared chisq_dist_log(1);
        double logp = std::log(boost::math::cdf(complement(chisq_dist_log, stat)));
  ...
  if(!std::isnan(StdStat) && (StdStat > m_SPA_Cutoff) && m_traitType != "quantitative"){
   ...
   if(t_isCondition){
    ...
    if (pval_noSPA_c != 0){
        ...
        boost::math::chi_squared chisq_dist_log(1);
	double logp_c = std::log(boost::math::cdf(complement(chisq_dist_log, stat_c)));
