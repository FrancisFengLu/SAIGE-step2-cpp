// Standalone port of UTIL.cpp for Step 2
// Shared utility functions -- all R/Rcpp dependencies removed
...
// Thread-local RNG for nb() function
static thread_local std::mt19937 s_util_rng(42);

arma::vec nb(unsigned int n){
  // Replacement for Rcpp::rbinom(n,1,0.5)
  // Generates n random 0/1 values with probability 0.5
  arma::vec result(n);
  std::bernoulli_distribution dist(0.5);
  for(unsigned int i = 0; i < n; i++){
    result(i) = dist(s_util_rng) ? 1.0 : 0.0;
  }
  return result;
}
...
{
  if(t_wBeta.size() != 2)
    throw std::runtime_error("The size of argument t_wBeta should be 2.");
  ...
  if(t_kernel == "linear.weighted"){
    // Replacement for Rcpp::dbeta(freq, a, b)
    // boost::math::pdf(beta_distribution(a,b), x)
    boost::math::beta_distribution<> beta_dist(t_wBeta(0), t_wBeta(1));
    weights = boost::math::pdf(beta_dist, t_freq);
  }
