#!/usr/bin/env Rscript
# ==============================================================================
# SAIGE Step 2: R vs C++ Result Comparison (Sparse GRM model)
# ==============================================================================
# Compares R SPAGMMATtest output with C++ saige-step2 output
# for the quantitative sparse GRM example model.
#
# Usage: Rscript compare_step2_sparse.R
# ==============================================================================
...
r_file   <- file.path(base_dir, "test/R/r_step2_sparse_results.txt")
cpp_file <- file.path(base_dir, "test/output/cpp_compare_sparse_results.txt")
out_file <- file.path(base_dir, "test/RESULTS_step2_sparse.txt")

cat("=== SAIGE Step 2: R vs C++ Comparison (Sparse GRM) ===\n\n")
...
# --- Write to file -----------------------------------------------------------
sink(out_file)
cat("SAIGE Step 2: R vs C++ Comparison Results (Sparse GRM)\n")
cat(paste("Date:", Sys.time(), "\n"))
cat(paste("R results:", r_file, "\n"))
cat(paste("C++ results:", cpp_file, "\n"))
cat(paste("Model: example_quantitative_sparseGRM.rda (LOCO=FALSE, isFastTest=TRUE)\n"))
cat(paste(rep("=", 105), collapse = ""), "\n\n")
