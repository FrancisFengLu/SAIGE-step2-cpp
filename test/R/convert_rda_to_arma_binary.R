#!/usr/bin/env Rscript
# ==============================================================================
# convert_rda_to_arma_binary.R
# ==============================================================================
#
# Purpose:
#   Convert SAIGE's example_binary.rda null model to Armadillo binary files
#   (.arma) and a nullmodel.json, so the standalone C++ Step 2 binary can
#   load the exact same null model that R SAIGE Step 2 uses.
#
# Usage:
#   Rscript convert_rda_to_arma_binary.R
#
# Input:
#   SAIGE extdata example_binary.rda
#
# Output:
#   Directory: test/data/nullmodel_from_rda_binary/
...
cat("=== convert_rda_to_arma_binary.R ===\n\n")

# --- Paths -------------------------------------------------------------------

rda_path <- "/Users/francis/Desktop/Zhou_lab/SAIGE_gene_pixi/SAIGE/extdata/output/example_binary.rda"
output_dir <- "/Users/francis/Desktop/Zhou_lab/SAIGE_gene_pixi/Step_2_Feb_11/test/data/nullmodel_from_rda_binary"
...
if (is_loco) {
  ...
  # Use chromosome 1 for testing (matching genotype_100markers which is all chr 1)
  target_chr <- 1
...
# Extract offset (important for Firth logistic regression in binary traits)
if (!is.null(modglmm$offset)) {
  model_offset <- as.vector(modglmm$offset)
  cat("offset: present, length =", length(model_offset), "\n")
  cat("  offset range: [", min(model_offset), ",", max(model_offset), "]\n")
} else {
  model_offset <- NULL
  cat("offset: not present in model\n")
}
...
# For BINARY: mu2 = mu * (1 - mu)
if (traitType == "binary") {
  mu2 <- mu * (1 - mu)
  cat("Binary trait: mu2 = mu*(1-mu)\n")
  cat("  mu range: [", min(mu), ",", max(mu), "]\n")
  cat("  mu2 range: [", min(mu2), ",", max(mu2), "]\n")
...
# Write offset if present (needed for Firth correction in binary traits)
