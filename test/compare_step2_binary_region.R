#!/usr/bin/env Rscript
# ==============================================================================
# SAIGE Step 2: R vs C++ Binary Region-Based Result Comparison
# ==============================================================================
# Compares R SPAGMMATtest binary region output with C++ saige-step2 region output.
# Both must have been run with the SAME null model, genotype data, and group file.
#
# Usage: Rscript compare_step2_binary_region.R [r_results] [cpp_results] [output_file]
#   Defaults:
#     r_results:   test/R/r_step2_binary_region_results.txt
#     cpp_results: test/output/cpp_compare_binary_region_results.txt
#     output_file: test/RESULTS_step2_binary_region.txt
# ==============================================================================
...
r_file   <- if (length(args) >= 1) args[1] else file.path(base_dir, "test", "R", "r_step2_binary_region_results.txt")
cpp_file <- if (length(args) >= 2) args[2] else file.path(base_dir, "test", "output", "cpp_compare_binary_region_results.txt")
out_file <- if (length(args) >= 3) args[3] else file.path(base_dir, "test", "RESULTS_step2_binary_region.txt")

cat("=== SAIGE Step 2: R vs C++ Binary Region Comparison ===\n\n")
...
for (col in compare_cols) {
  ...
  if (any(cats %in% c("DIFFER", "NEAR", "CLOSE"))) {
    worst_idx <- which(cats %in% c("DIFFER", "NEAR", "CLOSE"))
...
cat(sprintf("%-16s | %7s | %7s | %7s | %7s | %7s | %7s | %12s | %12s\n",
            "Column", "Total", "EXACT", "MATCH", "CLOSE", "NEAR", "DIFFER", "MaxRelDiff", "MaxAbsDiff"))
cat(paste(rep("-", 110), collapse = ""), "\n")
for (i in seq_len(nrow(results))) {
  r <- results[i, ]
  cat(sprintf("%-16s | %7d | %7d | %7d | %7d | %7d | %7d | %12.3e | %12.3e\n",
              r$Column, r$N_compared, r$N_EXACT, r$N_MATCH, r$N_CLOSE, r$N_NEAR, r$N_DIFFER,
              r$Max_RelDiff, r$Max_AbsDiff))
}

# Print details for disagreements
  ...
  for (col in names(all_details)) {
    ...
    for (j in seq_len(nrow(d))) {
      cat(sprintf("  %s: R=%.10e  C++=%.10e  relDiff=%.3e  absDiff=%.3e\n",
                  d$Key[j], d$R_value[j], d$Cpp_value[j], d$RelDiff[j], d$AbsDiff[j]))
    }
  }
}
...
cat(sprintf("NEAR   (rel diff < 1e-2):  %d\n", sum(results$N_NEAR)))
cat(sprintf("DIFFER (rel diff >= 1e-2): %d\n", total_differ))

if (total_differ == 0 && total_near == 0) {
  cat("\nOVERALL: PASS -- All values match within tolerance (< 1e-4).\n")
} else if (total_differ == 0) {
  cat(sprintf("\nOVERALL: NEAR PASS -- %d values in NEAR range (1e-4 to 1e-2), none beyond 1e-2.\n", total_near))
} else {
  cat(sprintf("\nOVERALL: FAIL -- %d values differ beyond 1e-2 tolerance.\n", total_differ))
}

# --- Write to file -----------------------------------------------------------
sink(out_file)
cat("SAIGE Step 2: R vs C++ Binary Region Comparison Results\n")
...
cat(sprintf("\nTotal comparisons: %d\n", total_compared))
cat(sprintf("EXACT: %d  MATCH: %d  CLOSE: %d  NEAR: %d  DIFFER: %d\n",
            sum(results$N_EXACT), sum(results$N_MATCH), sum(results$N_CLOSE),
            sum(results$N_NEAR), sum(results$N_DIFFER)))
if (total_differ == 0 && total_near == 0) {
  cat("OVERALL: PASS\n")
} else if (total_differ == 0) {
  cat(sprintf("OVERALL: NEAR PASS (%d in NEAR range)\n", total_near))
} else {
  cat(sprintf("OVERALL: FAIL (%d differ beyond 1e-2)\n", total_differ))
}
sink()
